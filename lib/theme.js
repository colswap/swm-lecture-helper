...
(function () {
  ...
  async function setMode(mode) {
    try {
      if (window.SWM && typeof window.SWM.saveSettings === 'function') {
        await window.SWM.saveSettings({ themeMode: mode });
      } else {
        const { settings = {} } = await chrome.storage.local.get('settings');
        await chrome.storage.local.set({ settings: { ...settings, themeMode: mode } });
      }
    } catch (e) {
  ...
  // 외부에서 접근 가능하게 글로벌 노출
  window.SWMTheme = { getMode, setMode, cycleMode, applyResolved, LABELS, MODES, PREFIX };
})();
