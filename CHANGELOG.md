...
### 🐛 회귀 수정
- **welcome.html 버전 배지 동적화** — manifest 버전을 자동 반영해 릴리스마다 수동 수정할 필요 없음 (회귀 반복 차단)
- **"마감 포함" 시간표 동기화 UX** — `마감 포함` 선택 시 `이번 주` 토글도 자동 해제돼 과거 강연이 바로 보임 (이전: 상태 필터만 해제돼 여전히 이번 주만 표시)
- **즐겨찾기 블록 제목 겹침** — 제목이 길어질 때 우상단 ★ 와 겹치던 레이아웃 수정

### 🧹 UX 문구 다듬기
- storage 포화 알림: "저장 용량 초과 — 오래된 캐시 정리 필요" → "저장 공간이 가득 찼어요. 확장의 데이터 초기화 메뉴를 사용해 주세요."
- Gcal 중복 추가 안내: Console 명령 스니펫 노출 제거 (일반 사용자 대상에 부적합)
- 동기화 실패 토스트: "실패 (상세 콘솔)" → "알 수 없는 오류 — 잠시 후 다시 시도하세요"
- ⋯ 메뉴 "팔레트 테마" 툴팁: "(벚꽃/파스텔/다크 플랜 등)" → "시간표 블록 색상 테마 선택"

### 🔧 내부 정리
- `MAX_PAGES` 동일 이름 다른 값 해소 → `MAX_APPLIED_PAGES` (20) / `MAX_LIST_PAGES` (300) 분리
- 주석 정비: "v1.15.0 NLS v2" → "NLS v3 (BM25)"

### 📊 품질
- 335/335 테스트 통과 (변경 없음, 회귀 방지)
